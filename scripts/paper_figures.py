...
def main() -> None:
    ...
    ax.axhline(0.015, color="grey", ls="--", lw=1, label="1.5% threshold")
    for i, m in enumerate(masses):
        ax.text(i, m + 0.001, f"{m:.2%}", ha="center", fontsize=9)
    ax.set_ylabel("honest_report mass (summed over layers)")
    ax.set_title("Vocabulary-matched control")
    ax.legend()
    fig.tight_layout()
    ...
    fig.suptitle("Per-word attribution — honest_report mass at the commit point")
    fig.tight_layout()
    ...
    ax.set_xlabel("position relative to commit (0 = commit)")
    ax.set_ylabel("honest_report mass, L26")
    ax.set_title("Honest mass by position in the tail preceding the commit")
